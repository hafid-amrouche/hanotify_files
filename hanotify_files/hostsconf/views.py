...
def render_product(request, slug, product_id):
    ...
    context={
        'store': json.dumps(store_data),
        'favicon': store_data.get('favicon') or "",
        'lang': store_data.get('language'),
        'dir': 'rtl' if store_data.get('language') == 'ar' else 'ltr',
        'url': request.build_absolute_uri(),
    }
    ...
    if product:
        context.update({
            ...
            'description': product.get('miniDescription') or store_data.get('description'),
            'image': product.get('galleryImages')[0] if product.get('galleryImages') else (store_data.get('logo') or ""),
        })
